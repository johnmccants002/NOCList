...
print("\(calcComp()) agents have been compromised!")
...
func findCleanAgents () -> [(String, String, Int, Bool)] {
    ...
    var cleanAgentsCoverNames = [String]()
    for agent in agents {
        if agent.compromised == false {
            cleanAgents.append(agent)
            cleanAgentsCoverNames.append(agent.coverName)
        }
        
    }
    print(cleanAgentsCoverNames)
    return cleanAgents
}
...
func sortLevels() {
    ...
    var lowLevelAgents = [(coverName: String, realName: String, accessLevel: Int, compromised: Bool)]()
    var midLevelAgents = [(coverName: String, realName: String, accessLevel: Int, compromised: Bool)]()
    var highLevelAgents = [(coverName: String, realName: String, accessLevel: Int, compromised: Bool)]()
    ...
    for agent in agents {
       ...
       print("\(lowLevelAgents.count) low level agents, \(midLevelAgents.count) mid level agents, \(highLevelAgents.count) high level agents")
...
func printCoverAndAccessLevels () {

    for currlevel in 1...10 {
...
printCoverAndAccessLevels()

/*    for agent in agents {
    if agent.accessLevel <= currentLevel {
        orderedAgents.append(agent)
        }
